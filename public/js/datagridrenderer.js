...
// Latex table
//---------------------------------------
latextable: function (dataGrid, headerNames, headerTypes, indent, newLine) {
    //inits...
    var outputText = "";
    var numRows = dataGrid.length;
    var numColumns = headerNames.length;
    ...
    // header row
    if (headerNames[0] != "val0") {
      outputText += "\\begin{tabular}{";
      for (var j=0; j < numColumns; j++) {
        if (j < (numColumns-1)) {outputText+="l"};
      };
      outputText += "l} \\\\" + newLine;
      outputText += "\\toprule" + newLine;
      outputText += indent+" ";
      for (var j=0; j < numColumns; j++) {
        str = headerNames[j];
        newstr = str.replace(re, "\\_");
        outputText += " " + newstr;
      // old code  outputText += " " + headerNames[j];
        if (j < (numColumns-1)) {outputText+=" &"};
      };
      outputText += " \\\\" + newLine;
      outputText += "\\midrule" + newLine;
    };
    ...
    for (var i=0; i < numRows; i++) {
      ...
      for (var j=0; j < numColumns; j++) {
        ...
        newstr = str.replace(re, "\\_");
        outputText += " " + newstr;
     // old code   outputText += dataGrid[i][j];
        if (j < (numColumns-1)) {outputText+=" & "};
      };
      outputText += " \\\\" + newLine;
    };
    ...
    // bottom row
    outputText += "\\bottomrule" + newLine;
    outputText += "\\end{tabular}" + newLine;
